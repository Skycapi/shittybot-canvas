...
async function playlistCard({ songs }) {
  ...
  const generateSingleCard = async (songsBatch, batchIndex) => {
    ...
    for (let i = 0; i < songsBatch.length; i++) {
      ...
      ctx.quadraticCurveTo(
        ...
        yPosition,
        cardWidth - padding,
        yPosition + borderRadius
      );
      ctx.lineTo(cardWidth - padding, yPosition + songHeight - borderRadius);
      ctx.quadraticCurveTo(
        cardWidth - padding,
        yPosition + songHeight,
        cardWidth - padding - borderRadius,
        yPosition + songHeight
      );
      ...
      ctx.quadraticCurveTo(
        ...
        yPosition + songHeight,
        padding,
        yPosition + songHeight - borderRadius
      );
      ...
      ctx.fillText(
        (batchIndex * maxSongs + i + 1).toString(),
        padding + 30,
        yPosition + 50
      );
      ...
      ctx.lineTo(
        padding + 80 + thumbnailSize,
        yPosition + 10 + thumbnailSize - borderRadius
      );
      ctx.quadraticCurveTo(
        padding + 80 + thumbnailSize,
        yPosition + 10 + thumbnailSize,
        padding + 80 + thumbnailSize - borderRadius,
        yPosition + 10 + thumbnailSize
      );
      ...
      ctx.drawImage(
        loveIcon,
        cardWidth - padding - iconSize - 20,
        yPosition + (songHeight - iconSize) / 2,
        iconSize,
        iconSize
      );
    }
  ...
  for (let i = 0; i < songs.length; i += maxSongs) {
    ...
    const buffer = await generateSingleCard(
      songsBatch,
      Math.floor(i / maxSongs)
    );
    buffers.push(buffer);
  }
